...
int PonConectado(ListaConectados *lista, char nombre[80], int socket){
	if (lista -> num == 100){
		return -1;
	}
	else{
...
void *AtenderCliente (void *socket){
	...
	while (terminar == 0)
	{
		//Recogemos la peticion. Hacemos una operaciÃ³n de lectura a travÃ©s del socker de conexiÃ³n y la peticiÃ³n la guarda en el vector de caracteres peticiÃ³n
		...
		if (codigo == 0) 
		{
			...
			//QUITAMOS UN CONECTADO A LA LISTA.
			if (contador != 0)
			{
				EliminarConectado(&miListaConectados, socket);
				printf("Eliminado correctamente de la ListaConectados.\n");
				//printf("%d\n", &miListaConectados.conectados[contador].socket);
				contador = contador -1;
				close(socket);
			}
			 //MODIFICADO 04/20 22:45
		}
		else if(codigo==1) //Peticion 1  //"MENSAJE ESPERADO:   1/usuari/contrasena --> usuari: contrasena:
		{
			...
			else
			{
				...
				else if ((strcmp(row[0], Usuario) == 0) && (strcmp(row[1], Password) == 0)) 
				{
					...
					printf ("Usuario del Jugador: %s y Socket: %d\n", row[0], &miListaConectados.conectados[contador].socket);
					sprintf(respuesta, "Correcto/%d", &miListaConectados.conectados[contador].socket);
					
					//PONEMOS UN CONECTADO A LA LISTA.
					int x = PonConectado(&miListaConectados, Usuario , &miListaConectados.conectados[contador].socket);
					if (x == -1)
		...
		else if(codigo==3) //Peticion 3  //"MENSAJE ESPERADO:   3/
		{
			...
			sprintf(respuesta, "%d/%s\0", contador, mensaje);
			printf("%s\n", respuesta);
		}
...
int main(int argc, char *argv[]) 
{
	...
	for(;;)
	{
		...
		miListaConectados.conectados[contador].socket = sock_conn;
		printf("%d\n", &miListaConectados.conectados[contador].socket);
		printf("ha salido bien\n");
		pthread_create (&thread, NULL, AtenderCliente, &miListaConectados.conectados[contador].socket);	
